...
describe("The calculateBonus(multiplier) method:", () => {
  ...
  describe("Manager.totalSubSalary()", () => {
    it("should be called in calculateBonus()", () => {
      const spy = chai.spy.on(splinter, "totalSubSalary");
    ...
    context(
      ...
      () => {
        it("should recursively call itself adding all sub-employee's salaries to a running sum", () => {
          const totalSubSalary = splinter.totalSubSalary(); // => splinter's only employee is leo, but leo may have employees
    ...
    context(
      ...
      () => {
        it("should add the employee's salary to a running sum", () => {
          const totalSubSalary = leo.totalSubSalary();
